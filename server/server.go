...
func Login(username string, password string, sess *sessions.Session) error {
	db, err := sql.Open("mysql", "ants:smschonrock12!@tcp(127.0.0.1:3306)/ants")
	if err != nil {
		panic(err)
	}
...
func main() {
	var err error
	store, err = mysqlstore.NewMySQLStore("ants:smschonrock12!@tcp(127.0.0.1:3306)/ants?parseTime=true&loc=Local", "sess", "/", 3600, []byte("MySecret"))
	if err != nil {
		panic(err)
	}
	defer store.Close()
